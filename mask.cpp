...
public:
    ...
    void TO_SEARCH(string pattern, const string& comparisonType) {
        found.clear();
        ...
        for (const auto& row : memory) {
            ...
            string cleanPattern = "";
            for (char ch : pattern) {
                if (ch != '*') cleanPattern += ch;
            }

            if (comparisonType == "=") {
                found.push_back(match ? "+" : "-");
            }
            else if (comparisonType == ">" && row > cleanPattern) comparisonResult = true;
            else if (comparisonType == ">=" && row >= cleanPattern) comparisonResult = true;
            else if (comparisonType == "<" && row < cleanPattern) comparisonResult = true;
            else if (comparisonType == "<=" && row <= cleanPattern) comparisonResult = true;
            else if (comparisonType == "<>" && row != cleanPattern) comparisonResult = true;
            ...
            if (comparisonType != "=") {
                found.push_back(comparisonResult ? "+" : "-");
            }
        }

        TO_SHOW(true, pattern, comparisonType);
    }
